...
namespace Serilog.Sinks.Batch
{
    ...
    internal abstract class BatchProvider : IDisposable
    {
        ...
        protected BatchProvider(int batchSize = 100, int maxBufferSize = 25_000)
        {
            ...
            _timerTask     = Task.Factory.StartNew(TimerPump, _cancellationTokenSource.Token, TaskCreationOptions.LongRunning, TaskScheduler.Current);
            _eventPumpTask = Task.Factory.StartNew(EventPump, _cancellationTokenSource.Token, TaskCreationOptions.LongRunning, TaskScheduler.Current);
