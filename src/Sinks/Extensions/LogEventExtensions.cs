    ...
    internal static class LogEventExtensions
    {
            ...
            IFormatProvider formatProvider)
        {
            ...
            eventObject.Add("Timestamp", storeTimestampInUtc
                ? logEvent.Timestamp.ToUniversalTime().ToString("yyyy-MM-dd HH:mm:ss.fffzzz", formatProvider)
                : logEvent.Timestamp.ToString("yyyy-MM-dd HH:mm:ss.fffzzz", formatProvider));
        ...
        private static object Simplify(LogEventPropertyValue data)
        {
            ...
            if (str == null) return null;
            {
                try
                {
                    ...
                    if (!str.TypeTag.StartsWith("DictionaryEntry", StringComparison.OrdinalIgnoreCase) && !str.TypeTag.StartsWith("KeyValuePair", StringComparison.OrdinalIgnoreCase))
                        return str.Properties.ToDictionary(p => p.Name, p => Simplify(p.Value));
